...
It can be thought of as configuration geared towards Ganchos. The distinction between this and the **Plugin Configuration Files** are that the latter are geared towards 
...
### Optional
* `isEligibleForSchedule`: *Boolean*; when true, plugin will be ran by the scheduler on startup and then on the interval provided by the `runEveryXMinutes` configuration setting
...
*Note: that these settings below are the configuration options recognized and used by Ganchos and are optional; however a plugin can specify any configuration settings that it needs.*
...
* `runDelayInMinutes`: *Number*; if provided and great than 0, will wait for desired number of seconds (fractional values acceptable) before executing plugin logic 
* `runEveryXMinutes`: *Number*; if a plugin is marked as eligible for scheduling, this will be the value used for the waiting interval between executions. Fractional numbers are acceptable and if it's <= 0 scheduling will disabled for the plugin.
...
Ganchos should be robust enough to never fail, gracefully handle bad plugins and expose any potential problems via clear logging so that in the case of a plugin with a bug, 
